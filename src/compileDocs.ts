    ...
    cls.forEach((line: string, idx: number) => {
        ...
        if (/---@field/.test(line)) {
            ...
            while (cls[searchIndex].includes('---##')) {
                fieldDesc = `${cls[searchIndex].replaceAll(/(---|#)/g, '')}${fieldDesc}`.trimEnd();
                searchIndex--;
            }
...
functionGroups.forEach((fn: string[]) => {
    ...
    let generic = '';
    fn.forEach(line => {
        if (line.includes('---@generic T : ')) generic = line.split('---@generic T : ')[1];
        if (!line.startsWith('---#')) return;
        functionDesc = `${functionDesc}${line.replaceAll(/(---|#)/g, '')}`.trimStart();
    });
    ...
    let functionStr = `<a id="${label.toLowerCase()}"></a>\n## \`${displayName}\`\n${functionDesc}\n### Parameters:\n${
        fn
            ...
            .map(
                param =>
                    `- \`${param.split(' ')[1]}: ${param.split(' ')[2]}\` - ${param.split(' ').slice(3).join(' ')}`,
            )
            .join('\n') || '- None'
    }\n### Returns:\n${
        fn
            .filter(line => line.includes('@return'))
            .map(param => `- \`${param.split(' ')[1]}\` - ${param.split(' ').slice(3).join(' ')}`)
            .join('\n') || '- `nil`'
    }\n`;
    if (generic) functionStr = functionStr.replaceAll(': T', `: ${generic}`).replaceAll('`T`', `\`${generic}\``);
...
outputStr = outputStr.replaceAll(/\(lua:\/\/([a-zA-Z0-9]+)\)/g, (_, g) => `(#class-${g.toLowerCase()})`);
    ...
    .map(
        ([global, obj], idx) =>
            `### ${idx + 2}. [${global.charAt(0).toUpperCase()}${global.slice(
                ...
                obj.attributes
                    .map(attr => `    - [${attr.split('-').slice(1).join('.')}](#${attr.toLowerCase()})`)
                    .join('\n') || '    - None'
            }\n  - Functions:\n${obj.functions
                .map(fn => `    - [${fn.split('-').slice(1).join('.')}](#${fn.toLowerCase()})`)
                .join('\n')}`,
    )
    .join('\n')}`;
